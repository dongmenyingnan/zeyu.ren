...
<body>
	...
	<div>
		<div id="top">
			<div id="topleft">
				<div>
					...
					<h3>${exceptionarticle.exceptionarticle_title}</h3>
				</div>
				<div style="margin-top: 10px;">
					<a class="black fs-16 ml-40" href="#">${exceptionarticle.exceptionarticle_author}</a> <span
						class="fs-14" style="margin-left: 20px;">${exceptionarticle.create_time}
						发表</span>
				</div>
			</div>
		<%-- 	<div id="topright">
				<div>
					<form
						action="${pageContext.request.contextPath}/user/addAttentionArticle.action?article_id=${articleVo.article.article_id}"
						method="post">
						<button id="btnFollow" class="myBtn">收藏</button>
						<span><b>${articleVo.article.attentions }</b> 收藏</span>
					</form>

				</div>
				<div style="margin-top: 10px;">
					<button id="btnCollect" class="myBtn">浏览</button>
					<span><b>${articleVo.article.hits}</b> 浏览 </span>&nbsp;&nbsp;&nbsp;
		...
		<div id="center">
			<div id="leftpart">
				<div id="qbody">
					...
					<p>
					${exceptionarticle.exceptionarticle_content}
					</p>
					 </div>
				...
				<div id="qbody_date"  style="width: 70%;margin: 0 auto;">
				<div class="date">
				<span class="auther">作者编写于</span>
				<span style="margin-right: 10px;"><fmt:formatDate type="time" value="${exceptionarticle.exceptionarticle_time }" pattern="yyyy-MM-dd hh:mm:ss" /></span>
				</div>
				</div>
					</div>
				<%-- <div id="abody">
					<h4>
						<i class="i-status-being mr-10 answerIcon"></i> <span>${fn:length(articleVo.articleshowGathers)}</span>个评论
					</h4>
					<!-- 分割线-->
					<c:forEach items="${articleVo.articleshowGathers}" var="list">
		...
		<%-- 		<div id="answerPart">
					<h4>撰写评论</h4>
					<form
						action="${pageContext.request.contextPath}/replyArticle.action?article_id=${articleVo.article.article_id}"
						method="post">
						<textarea style="height: 200px;" rows="6" name="article_content"
							id="answer_content"></textarea>
						<button id="answerBtn" type="submit" class="myBtn">提交评论</button>
					</form>
				</div> --%>
