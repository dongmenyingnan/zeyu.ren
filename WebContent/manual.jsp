...
<body>
	...
	<c:forEach items="${list}" var="list">
		<li><a href="${pageContext.request.contextPath}${list.manual_url }?manualartcle_id=${list.manual_id}">${list.manual_name}</a></li>
		</c:forEach>
		...
		<div class="fy">
			...
			<c:forEach items="${Pages1}" var="pagesNew">
									<a ${pagesNew.style }
										href="${pageContext.request.contextPath}/display.action?page=${pagesNew.pageNum}&&flag=${pagesNew.flag}">${pagesNew.content }</a>
			</c:forEach>
			</div>
			</div>
