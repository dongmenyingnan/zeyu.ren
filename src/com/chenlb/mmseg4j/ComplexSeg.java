...
public class ComplexSeg extends Seg{
	...
	public Chunk seg(Sentence sen) {
		char[] chs = sen.getText();	//将检索词分成一个一个字符
		...
		CharNode[] cns = new CharNode[3];	//所有词都记录在第一个字的结点下.
		...
		if(!sen.isFinish()) {	//sen.getOffset() < chs.length
			...
			int maxLen = 0;
			offsets[0] = sen.getOffset();	//从第0个开始记录
			/*
			 * 遍历所有不同词长,还不是从最大到0(w[0]=maxLen(chs, offsets[0]); w[0]>=0; w[0]--)
			 * 可以减少一部分多余的查找.
			 */
			maxMatch(cns, 0, chs, offsets[0], tailLens, 0);	//匹配,同时找出长度.从 chs[offset] 开始找所有匹配的词, 找到的放到 tailLens[tailLensIdx] 中.同时把 chs[offset] 的字符结点保存在 cns[cnIdx].
